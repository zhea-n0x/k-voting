...
<style>
    ...
    <center>
        <div class="container px-36 mt-6">
            {{-- form detail --}}
            <form action="{{ route("privileged_admin/edit_dlm/") }}" method="POST" class="flex flex-col gap-3" enctype="multipart/form-data">
                @csrf
                @method('POST')
                {{-- show value from $bem --}}
                <center>
                    <img src="/uploads/dlm/{{ $dlm->photo }}" class="w-56 items-center" alt="" srcset="">
                </center>
                {{-- show image --}}
                <input type="hidden" value="{{ $dlm->id }}" name="id">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <input type="text" value="{{ $dlm->no_urut }}" class="w-full rounded-md" name="no_urut">
                </div>

                <div class="flex flex-col gap-3">
                    <label for="" class="text-left">Nama Calon</label>
                    <input type="text" value="{{ $dlm->name }}" class="w-full rounded-md" name="name">
                </div>
                <div class="flex flex-col gap-3">
                    <label for="" class="text-left">Program Studi</label>
                    <input type="text" value="{{ $dlm->study_program }}" class="w-full rounded-md" name="study_program">
                </div>
                <div class="flex flex-col gap-3">
                    <label for="" class="text-left">Visi</label>
                    <textarea name="visi" class="rounded-md">{{ $dlm->visi }}</textarea>
                </div>
                <div class="flex flex-col gap-3">
                    <label for="" class="text-left">Misi</label>
                    <textarea name="misi" class="rounded-md">{{ $dlm->misi }}</textarea>
                </div>
                {{-- save button --}}
                <div class="flex flex-col gap-3">
